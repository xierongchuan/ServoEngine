...
class MacdxSignalGenerator(BaseSignalGenerator):
    ...
    def generate(self, analysis: Dict, regime: Optional[Dict] = None) -> Dict:
        ...
        if macd_hist_prev <= 0 and macd_hist > 0:
            ...
            debug(f"[MACDX] Бычье пересечение: hist_prev={macd_hist_prev:.6f} <= 0, hist={macd_hist:.6f} > 0")
        if macd_hist_prev >= 0 and macd_hist < 0:
            macd_cross_short = True
            debug(f"[MACDX] Медвежье пересечение: hist_prev={macd_hist_prev:.6f} >= 0, hist={macd_hist:.6f} < 0")
